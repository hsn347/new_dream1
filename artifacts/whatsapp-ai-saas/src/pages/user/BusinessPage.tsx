...
function IconSnapchat({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      <path d="M12.206.793c.99 0 4.347.276 5.93 3.821.529 1.193.403 3.219.317 4.814l-.004.093c-.01.18-.018.345-.023.492.106.07.255.159.445.159.183 0 .398-.08.581-.255.172-.165.497-.242.78-.163.388.108.563.462.553.76-.008.276-.173.557-.479.719-.063.033-.066.043-.074.072-.023.082.018.171.162.305l.037.033c.415.35 1.396 1.178 1.253 2.542-.12 1.186-1.234 2.006-2.498 2.006-.195 0-.403-.02-.615-.067-.078.217-.166.476-.253.717-.143.393-.29.794-.465 1.131l.012.012c.354.308.922.773 1.676.773.25 0 .5-.063.739-.186.112-.057.237-.076.348-.076.265 0 .569.126.752.344.251.302.281.67.075.985-.313.481-1.205.797-2.111.797-.381 0-.722-.062-1.015-.19l-.037-.016c-.427-.194-.747-.396-1.003-.547-.337-.2-.574-.323-.78-.323-.204 0-.432.12-.764.323-.258.152-.58.358-1.012.553-.302.134-.647.197-1.033.197-.898 0-1.78-.317-2.09-.797-.202-.316-.172-.682.08-.985.182-.219.485-.345.749-.345.111 0 .237.02.348.076.238.124.488.187.738.187.757 0 1.327-.466 1.68-.774l.012-.012c-.176-.337-.324-.738-.467-1.131-.086-.241-.175-.5-.252-.717-.21.047-.417.067-.612.067-1.265 0-2.379-.82-2.498-2.006-.144-1.364.839-2.192 1.253-2.541l.037-.034c.144-.134.185-.223.162-.305-.008-.03-.011-.04-.074-.072-.304-.163-.471-.443-.479-.72-.01-.297.164-.65.552-.759.283-.079.607-.001.78.164.183.174.398.254.582.254.19 0 .339-.088.445-.158l-.023-.493c-.087-1.596-.212-3.621.317-4.814C7.86 1.07 11.216.793 12.206.793z"/>
    </svg>
  );
}
...
const SOCIAL_CONFIG: Record<string, {
  ...
  "سناب شات": { icon: IconSnapchat,  color: "text-[#FFFC00]", bg: "bg-[#FFFC00]/10", placeholder: "https://snapchat.com/add/username" },
};
...
const EMPTY_BUSINESS: Business = {
  ...
  phones: [],
  branches: [],
  socialLinks: { واتساب: "", إنستقرام: "", "تويتر / X": "", "سناب شات": "" },
  bankAccounts: [],
...
const SOCIAL_KEYS = ["واتساب", "إنستقرام", "تويتر / X", "سناب شات"];
    ...
    api.user.business.get()
      .then(data => {
        ...
        const sl = data.socialLinks && Object.keys(data.socialLinks).length
          ? data.socialLinks
          : { واتساب: "", إنستقرام: "", "تويتر / X": "", "سناب شات": "" };
